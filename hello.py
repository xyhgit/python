...
class User:
    ...
    def __init__(self,n,h,w):
        ...
        self.weight=w
    def display(self):
        print("大家好，我叫{}，身高{}，体重{}，颜值超高".format(self.name,self.height,self.weight))
def Hello_World():
    global yourname,yourheight,yourweight
    yourname=input("输入你的名字：")
    yourheight=input("输入你的身高：")
    yourweight=input("输入你的体重：")
...
def main():
    ...
    user=User(yourname,yourheight,yourweight)
    user.display()
    deviding_line()

    chooseIfo='''Input the number of the function you want to Run(quit is exit):
    1、study_number	
    2、study_list
    3、study_tuple
    4、study_dict
    5、study_set
    6、study_Some_functions
    7、study_Slice
    8、study_loop_select
